...
# Resolve paths relative to this script's location so the pipeline runs from
# anywhere (e.g. `python scripts/clean_eurepoc.py` from the repo root, or from
# inside scripts/). Raw input is read from data/raw, outputs go to data/processed.
...
RAW_PATH = os.path.join(RAW_DIR, "eurepoc_global_dataset_1_3.csv")
...
# Receiver category is deduplicated PER INCIDENT before exploding: the receiver
# dataset records one row per affected country, so a single multi-country
# incident (e.g. a global espionage campaign) can repeat the same category
# dozens of times and swamp a year's totals. Each incident is counted once per
# distinct category it touches, regardless of how many countries were affected.
def explode_field_dedup(frame, col):
    out = frame[["incident_id", "year", col]].copy()
    out[col] = out[col].astype(str).str.split(";").apply(lambda lst: list(set(x.strip() for x in lst)))
    out = out.explode(col)
    return out

receiver_category_long = explode_field_dedup(df, "receiver_category")
...
# Save outputs
df.to_csv(out("eurepoc_cleaned_2015_2024.csv"), index=False)
incident_type_long.to_csv(out("eurepoc_incident_type_long.csv"), index=False)
receiver_category_long.to_csv(out("eurepoc_receiver_category_long.csv"), index=False)
annual_summary.to_csv(out("eurepoc_annual_summary.csv"), index=False)
incident_type_by_year.to_csv(out("eurepoc_incident_type_by_year.csv"))
receiver_category_by_year.to_csv(out("eurepoc_receiver_category_by_year.csv"))
cleaning_log.to_csv(out("eurepoc_cleaning_log.csv"), index=False)
